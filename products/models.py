...
class Product(models.Model):
    ...
    def get_image_url(self):
        ...
        if not slug_name:  # 如果slugify后为空，使用商品ID
            slug_name = f'product-{self.id}'
            
        default_image = f'{settings.STATIC_URL}images/products/{slug_name}.png'
        for static_dir in settings.STATICFILES_DIRS:
            static_path = os.path.join(static_dir, 'images', 'products', f'{slug_name}.png')
            if os.path.exists(static_path):
                return default_image
                
        return f'{settings.STATIC_URL}images/no-image.png'
